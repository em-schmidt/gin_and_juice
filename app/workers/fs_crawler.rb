class FsCrawler
  ...
  def self.perform(fs_full_path, max_cache_age)
    Rails.logger.info "Starting crawl of path: #{fs_full_path}"
    Dir.chdir(fs_full_path) do 
      Dir.glob("**/*").map do |path| 
        if File.file?(path)
          ...
          if cache_entry.nil?
            Rails.logger.info "path: #{full_path} not in stat cache"
            ...
            if (Time.now.to_i - cache_create_time) > max_cache_age
              Rails.logger.info "path: #{full_path} cache_age: #{Time.now.to_i - cache_create_time} max_cache_age: #{max_cache_age}"
